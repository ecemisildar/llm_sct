...
class StuckRecovery(Node):
    ...
    def __init__(self):
        ...
        self.escape_angular_z = max(
            0.1, abs(float(self.declare_parameter("escape_angular_z", 1.0).value))
        )
        self.reverse_linear_x = -abs(
            float(self.declare_parameter("reverse_linear_x", -0.20).value)
        )
        self.reverse_duration_s = max(
            0.1, float(self.declare_parameter("reverse_duration_s", 0.8).value)
        )
        self.forward_linear_x = abs(
            float(self.declare_parameter("forward_linear_x", 0.20).value)
        )
        ...
        self.create_subscription(
            Twist, "cmd_vel_supervisor", self.command_callback, 10
        )
