...
class RobotSupervisor(Node):
    def __init__(self):
        ...
        self.sct_uncontrollable_log_path: Optional[Path] = None
        self.sct_uncontrollable_summary_path: Optional[Path] = None
        self.sct_event_counts: Dict[str, int] = {}
        self.sct_uncontrollable_counts: Dict[str, int] = {}
        ...
        self.enable_service_patrolling = self.create_service(
            SetBool,
            "enable_supervisor_patrolling",
            self.handle_enable_supervisor_patrolling,
        )
    ...
    def _init_sct_decision_log(self):
        ...
        self.sct_uncontrollable_log_path = (
            log_dir / f"triggered_uncontrollable_events_{self.ns}.csv"
        )
        self.sct_uncontrollable_summary_path = (
            log_dir / f"uncontrollable_event_percentages_{self.ns}.csv"
        )

        if not self.sct_decision_log_path.exists():
        ...
        if not self.sct_uncontrollable_log_path.exists():
            with self.sct_uncontrollable_log_path.open("w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "wall_time",
                    "robot",
    ...
    def _log_sct_decision(self, selected_event: str, ce_exists: bool, snapshot):
        ...
        if self.sct_uncontrollable_log_path is not None and uncontrollable_events:
            wall_time = time.time()
            with self.sct_uncontrollable_log_path.open("a", newline="") as f:
                writer = csv.writer(f)
                for event in uncontrollable_events:
                    self.sct_uncontrollable_counts[event] = (
                        self.sct_uncontrollable_counts.get(event, 0) + 1
                    )
                    writer.writerow([
                        f"{wall_time:.6f}",
                        self.ns,
    ...
    def _write_sct_event_summary(self):
        ...
        if self.sct_uncontrollable_summary_path is not None:
            uncontrollable_total = sum(self.sct_uncontrollable_counts.values())
            with self.sct_uncontrollable_summary_path.open("w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "robot",
                    "uncontrollable_event",
                ...
                for event, count in sorted(self.sct_uncontrollable_counts.items()):
                    percentage = (
                        100.0 * count / uncontrollable_total
                        if uncontrollable_total
                        else 0.0
                    )
                    writer.writerow([self.ns, event, count, f"{percentage:.3f}"])
                writer.writerow([
                    self.ns,
                    "TOTAL",
                    uncontrollable_total,
                    "100.000" if uncontrollable_total else "0.000",
                ])
    ...
    def handle_enable_supervisor_patrolling(self, request, response):
        if bool(request.data):
            ok, detail = self._switch_mission("patrolling")
            if not ok:
        ...
        response.success = True
        if self.enabled:
            response.message = f"Supervisor enabled for patrolling ({detail})."
        else:
            response.message = "Supervisor disabled."
        return response
