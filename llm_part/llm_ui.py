...
class LLMTaskApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        ttk.Label(frame, text="Control task").grid(row=0, column=0, sticky="w")
        ttk.Label(
            frame,
            text="Describe the plants and control requirements the LLM should model.",
        ).grid(row=1, column=0, sticky="w", pady=(2, 6))

        self.task_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, height=12)
    ...
    def start_generation(self) -> None:
        ...
        self._set_output("")
        threading.Thread(
            target=self._generate_worker, args=(task, model), daemon=True
        ).start()
