"""Synchronize each prompt's best saved supervisor with target_approach_nomark."""
import csv
import json
import statistics
import sys
...
OUTPUT = BASE / 'best_patrolling_target_approach_nomark'
OUTPUT.mkdir(exist_ok=True)
SPEC = ROOT / 'automata/baseline_automata/patrolling/target_approach_nomark.xml'
...
for prompt in range(1, 7):
    ...
    if synthesized.states:
        output_yaml = folder / f'S_prompt_{prompt}_target_approach_nomark.yaml'
...
(OUTPUT / 'README.md').write_text('Best patrolling supervisors synchronized with target_approach_nomark.xml.\n\n'
    ...
    'For each prompt: K = Sync(original plant, evaluated supervisor, target_approach_nomark); S = SupC(original plant, K). '
